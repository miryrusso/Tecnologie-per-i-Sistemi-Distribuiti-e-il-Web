...
int main(int argc, char **argv){
    ...
    while(1){
        ...
        if(strlen(buffer) > strlen(longword)){
            strcpy(longword, buffer); 
        }

        longword[strcspn(longword, "\0")] = '\n'; 

        write(connection, &longword, strlen(longword)); 
        close(connection);
    }
